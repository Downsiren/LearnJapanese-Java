function deleteAnn(id){
    layer.confirm('是否删除？', {icon: 3, title:'提示'}, function(index){
        //执行删除
        $.ajax({
            url : contextPath + '/manage/annController/deleteAnn',
            timeout : 300000,
            dataType : "json",
            type : "post",
            data : {
...
function addAnn(){
    layer.open({
        ...
        area: ['100%', '100%'],  //大小
        content: contextPath + '/manage/annController/Ann_add'
    });
}

//打开公告详情页面
function queryAnnDetails(id){
    layer.open({
        type: 2,
        title: '查看公告详情',
        scrollbar: false, 	//父页面是否有滚动条
        ...
        maxmin: false,		//是否显示最大化按钮
        area: ['100%', '100%'],  //大小
        content: contextPath + '/manage/annController/ann_details?id='+id
    });
}
